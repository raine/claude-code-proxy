...
impl Provider for GrokProvider {
    ...
    fn supported_models(&self) -> Vec<String> {
        GROK_MODELS.iter().map(|s| s.to_string()).collect()
    }
    ...
    async fn handle_messages(&self, body: MessagesRequest, ctx: RequestContext) -> Response {
        ...
        let previous_response_id_enabled = config::grok_previous_response_id();
...
fn map_xai_error_to_response(err: &client::GrokError) -> Response {
...
pub(crate) struct GrokCli;

impl CliHandlers for GrokCli {
    fn login(&self) -> Result<(), anyhow::Error> {
        println!(
            "xAI SuperGrok OAuth (browser PKCE via OIDC discovery). For headless hosts use: grok auth device"
        );
    ...
    fn status(&self) -> Result<(), anyhow::Error> {
        ...
        let api_key = config::grok_api_key().is_some();
...
pub(crate) static GROK_CLI: GrokCli = GrokCli;
